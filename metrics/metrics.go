...
func (s *ConnectionStats) toBigQuery() *connectionStats {
	cr := closeReason{}
	if _, ok := s.CloseError.(*quic.StatelessResetError); ok {
		cr.StatelessReset = bigquery.NullBool{Bool: true, Valid: true}
	} else if _, ok := s.CloseError.(*quic.VersionNegotiationError); ok {
		cr.VersionNegotiation = bigquery.NullBool{Bool: true, Valid: true}
	} else if _, ok := s.CloseError.(*quic.IdleTimeoutError); ok {
		cr.Timeout = bigquery.NullString{StringVal: "idle", Valid: true}
	} else if _, ok := s.CloseError.(*quic.HandshakeTimeoutError); ok {
		cr.Timeout = bigquery.NullString{StringVal: "handshake", Valid: true}
	} else if appErr, ok := s.CloseError.(*quic.ApplicationError); ok {
		cr.ApplicationError = &transportOrApplicationError{
			Remote:    appErr.Remote,
			ErrorCode: int64(appErr.ErrorCode),
		}
	} else if tErr, ok := s.CloseError.(*quic.TransportError); ok {
		cr.TransportError = &transportOrApplicationError{
			Remote:    tErr.Remote,
			ErrorCode: int64(tErr.ErrorCode),
		}
	}
